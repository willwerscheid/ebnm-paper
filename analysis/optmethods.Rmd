...
I only consider optimization methods `nlm`, `nohess_nlm`, and `trust` here, since previous tests suggested that these were the only consistently competitive methods. I do a more careful comparison here, with sixteen possible scenarios:
...
plot_one_n <- function(res) {
  res <- res %>%
    ...
    mutate(x = fct_relevel(x, "PN", "PL", "PN (est mu)", "PL (est mu)"),
           y = fct_relevel(y, "Hetero, null case", "Hetero, non-null", 
                           "Homo, null case", "Homo, non-null"))
